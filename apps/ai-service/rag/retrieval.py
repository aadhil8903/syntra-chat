...
def search_documents_vector(
    ...
    # Strict separation: only retrieve vectors generated by the active embedding provider
    if embedder.provider_name == "bge_local":
        provider_filter = {
            "$or": [
                {"embedding_provider": "bge_local"},
                {"embedding_provider": {"$exists": False}},
            ]
        }
    else:
        provider_filter = {"embedding_provider": embedder.provider_name}

    # Build filter based on provided document_ids and active provider
    ...
    # Validate embedding dimensionality matches active embedder to prevent shape mismatch
    chunks = [c for c in cursor if len(c.get("embedding", [])) == embedder.dimension]

    if not chunks:
        logger.warning(f"No valid chunks found for document_ids {document_ids} using provider '{embedder.provider_name}'")
        return []
