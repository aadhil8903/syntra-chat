      ...
      <div class="flex flex-wrap items-center gap-1.5">
        @for (file of uniqueSourceFiles; track file.filename; let idx = $index) {
          <div
            ...
            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#111114] hover:bg-[#18181b] border border-[#27272a] hover:border-zinc-500 text-xs text-zinc-200 transition-all cursor-pointer select-none max-w-full truncate group"
            [title]="'Referenced: ' + file.filename"
          >
            <span class="text-zinc-500 text-[10px] group-hover:text-zinc-300">
              {{ file.sourceType === 'tabular' ? '📊' : '📄' }}
            </span>
            <span class="font-medium truncate text-zinc-300 group-hover:text-white text-[11px]">{{ file.filename }}</span>
            @if (file.pages.length > 0) {
              <span class="text-[10px] text-[#71717a] font-mono group-hover:text-zinc-400">
                p. {{ file.pages.join(', ') }}
              </span>
            }
          </div>
